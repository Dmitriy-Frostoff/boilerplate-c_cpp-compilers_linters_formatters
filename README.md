...
│
├──  build            # built binaries of the project (**note**: `build/compile_commands.json` are strongly
│                     # required for linter!);
│
├── configs # the folder includes config and ignore files for: `ESlint`, `Prettier`, `Commitlint` packages.
│   │       # Currently about ignore files: `node_modules` and a few more folders are ignored
...
│                 # Check the scripts (especially, the paths for linting/prettier'ing.
│                 # Currently: `'./src'`). Scripts already have CLI prefixes to link with config
│                 # and ignore files;
├── package-lock.json
├── README.md
└── usefull_chunks  # (optional!) the folder with usefull files and settings for `git`
    │               # (`msys2` version) and for adding quick access to the `ucrt64 terminal`
    │               # from `right-click` anywhere and examples of custom `bash scripts` for compiling `.exe`
    │               # of your project (**@note:** cwd will be is from where `bash script` was executed!).
    ├── build .exe files via bash
    │   ├── build_win_clang_gcc.sh
    │   └── build_win_gcc.sh
    ├── for git
    │   ├── .bashrc
    │   ├── .gitconfig
    │   └── git-prompt.sh
    └── Win10 x64 ucrt64 terminal shell
        └── msys2_ucrt64_shell.reg
```
...
> [!NOTE]  
> All the pathes to the tools are as alternative setup for Windows 10x64.
> if the `msys2` (at `C:/Tools/msys64/ucrt64`) with tools mentioned above is used, won't install
> anything from the below, but just use them!
